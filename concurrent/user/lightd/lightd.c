...
static int scale_light_intensity(float intensity);
static int signal_light_event(float intensity);
...
static int poll_sensor_data(struct sensors_poll_device_t *sensors_device)
{
	...
	/* emulation */
		...
		signal_light_event(cur_intensity);
	} else {
...
static int scale_light_intensity(float intensity)
{
	...
	return (100 * i + 100 * (intensity - i));
}

static int signal_light_event(float intensity)
{
	struct light_intensity *p = NULL;

	p = (struct light_intensity *)calloc(1,
				sizeof(struct light_intensity));

	if (!p)
		return -1;

	p->cur_intensity = scale_light_intensity(intensity);

	if (syscall(__NR_light_evt_signal, p) < 0) {
		printf("error: %s\n", strerror(errno));
		return -1;
	}
