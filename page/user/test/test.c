...
int main(int argc, char **argv)
{
	 ...
	 * memory mapping
	 */
	pgd_mmsize = PTRS_PER_PGD * sizeof(unsigned long);
	fake_pgd = mmap(NULL, pgd_mmsize, PROT_READ | PROT_WRITE,
			MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	if (fake_pgd < 0) {
		printf("Error mmap for fake pgd: %s\n", strerror(errno));
		return -1;
	}

	pmd_mmsize = PTRS_PER_PGD * PTRS_PER_PMD * sizeof(unsigned long);
	fake_pmds = mmap(NULL, pmd_mmsize, PROT_READ | PROT_WRITE,
			MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	if (fake_pmds < 0) {
		printf("Error mmap for fake pmds: %s\n", strerror(errno));
		ret = -1;
		goto unmap_pgd;
	}

	page_size = (1UL << page_shift);
	pte_mmsize = PTRS_PER_PGD * PTRS_PER_PMD * page_size;
...
unmap_pmd:
	munmap((void*)fake_pmds, pmd_mmsize);
unmap_pgd:
	munmap((void*)fake_pgd, pgd_mmsize);
